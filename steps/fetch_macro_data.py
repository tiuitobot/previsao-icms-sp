...
def main(*, output_dir: str = "", **kwargs) -> dict:
    ...
    # 3. Focus expectations from BCB Olinda
    current_year = datetime.now().year
    focus_url = (
        f"https://olinda.bcb.gov.br/olinda/servico/Expectativas/versao/v1/odata/"
        f"ExpectativasMercadoAnuais?$filter=DataReferencia%20eq%20'{current_year}'"
    ...
    for indicator in ["PIB Total", "IGP-M", "IPCA", "Selic"]:
        ...
        if len(match) > 0:
            focus_expectations[indicator] = float(match.iloc[0]["Mediana"])
    ...
    result = {
        ...
        "data_freshness": {
            ...
            "focus_reference_year": current_year,
        },
        "status": "ok"
    }
